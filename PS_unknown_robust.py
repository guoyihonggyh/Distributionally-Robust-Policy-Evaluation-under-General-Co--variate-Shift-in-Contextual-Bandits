...
def train(dataname,lr,batch_size):
    ...
    def sample_logging_data(xy, label, size, pdf, pdf_eval, logging_policy, eval_policy):
        logging_index = np.zeros(size + n_class, np.int)
        logging_action = np.zeros(size+ n_class, np.int)
        logging_reward = np.zeros(size+ n_class, np.int)
        index_list = [i for i in range(len(xy))]

        for i in range(size + n_class):
        ...
        for logging_interval in logging_policy_list:
            for tgt in target_policy:
                ...
                for _ in tqdm(range(20)):
                    ...
                    eval_index, eval_action, eval_reward = \
                        sample_eval_data(test_xy, test_label, 10, pdf, pdf_eval, estimated_loggign_policy, eval_policy)
                    ...
                    for size in size_list:
                        eval_index, eval_action, eval_reward = \
                            sample_eval_data(test_xy, test_label, size, pdf, pdf_eval, estimated_loggign_policy, eval_policy)
...
for dataname in datalist:
    for lr in lrlist:
        for batchsize in batchsize_list:
            ...
            savepath = os.path.join('tune', dataname + '_' + str(lr) + '_' + str(batchsize)) + 'unknownips_onehone.csv'
            frame.to_csv(savepath)

# satimage 1 [16,64,256]
