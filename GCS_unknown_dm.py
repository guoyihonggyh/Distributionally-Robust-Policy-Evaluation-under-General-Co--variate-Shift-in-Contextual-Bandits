...
def train(dataname,lr,batch_size):
    ...
    def sample_logging_data(xy, label, size, pdf, pdf_eval, logging_policy, eval_policy):
        ...
        logging_index_extra = np.zeros(n_class, np.int)
        logging_action_extra = np.zeros(n_class, np.int)
        logging_reward_extra = np.zeros(n_class, np.int)
        for i in range(n_class):
            t_index = size - 1
            logging_index_extra[i] = t_index
            action = i
            logging_action_extra[i] = action
            logging_reward_extra[i] = int(action == label[t_index])

        logging_index = np.concatenate((logging_index,logging_index_extra))
        logging_action = np.concatenate((logging_action,logging_action_extra))
        logging_reward = np.concatenate((logging_reward,logging_reward_extra))
                    ...
                    for _ in tqdm(range(10)):
                        ...
                        logging_index, logging_action, logging_reward = \
                            sample_logging_data(train_xy, train_label, bandit_train_size, pdf, pdf_eval, logging_policy,
                                                eval_policy)
                        clf = loggingpolicy(xy[logging_index], logging_action)
                        estimated_loggign_policy = clf.predict_proba(xy)
                        ...
                        eval_index_target_policy, eval_action_target_policy, eval_reward_target_policy = \
                            sample_eval_data_with_target_xshift(test_xy, test_label, size_list[0], pdf, pdf_eval,
                                                                logging_policy, eval_policy)
                        clf = p_xshift(xy[logging_index], xy[eval_index_target_policy])

                        pdfe = clf.predict_proba(xy)[:, 1].reshape(len(pdf), 1)
                        ...
                        model_robust_list, Myy_robust_list, Myx_robust_list = train_robust_regression(logging_index, logging_action,
                                                                                                      ...
                                                                                                      eval_reward_logging, \
                                                                                                      xy, label, pdfe, pdf_eval,
                                                                                                      estimated_loggign_policy, eval_policy, \
                                                                                                      dm=False, use_xshift=False)
                        for size in size_list:
                            eval_index_logging_policy, eval_action_logging_policy, eval_reward_logging_policy = \
                                sample_eval_data_with_logging_xshift(test_xy, test_label, size, pdf, pdf_eval, estimated_loggign_policy, eval_policy)
                            eval_index_target_policy, eval_action_target_policy, eval_reward_target_policy = \
                                sample_eval_data_with_target_xshift(test_xy, test_label, size, pdf, pdf_eval, estimated_loggign_policy, eval_policy)
                            res = evaluate_ips(logging_index, logging_action, logging_reward, \
                                               eval_index_logging_policy, eval_action_logging_policy, eval_reward_logging_policy, \
                                               xy, label, pdfe, pdf_eval, estimated_loggign_policy, eval_policy, GT)
...
for dataname in datalist:
    for lr in lrlist:
        for batchsize in batchsize_list:
            ...
            savepath = os.path.join('tune', dataname + '_' + str(lr) + '_' + str(batchsize)) + 'onehone_xshift_v0_dmunknown.csv'
