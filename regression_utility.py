...
def M_gradient_gpu(x, meanY, varY, y, Myy, Myx,d):
    # print(x)
    # bs = torch.shape(x)[0]
    bs = x.shape[0]
    # d = torch.shape(x)[1]
    d = x.shape[1]
    x = torch.reshape(x.detach(), (bs, d))
    y = torch.reshape(y.detach(), (bs,))
    meanY = torch.reshape(meanY.detach(), (bs,))
    varY = torch.reshape(varY.detach(), (bs, 1))

    y_vec = torch.cat([torch.reshape(y, (bs, 1)), x, torch.ones((bs, 1)).to(device)], axis=1)

    # emp = np.einsum('ij,i->ij', y_vec, y)
    emp = y_vec * torch.reshape(y,(bs,1))

    # print(np.shape(emp))
    # y_mean_vec = np.concatenate((np.reshape(meanY, (bs, 1)), x, np.ones((bs, 1))), 1)
    y_mean_vec = torch.cat([torch.reshape(meanY, (bs, 1)), x, torch.ones((bs, 1)).to(device)], 1)

    # y_var_vec = np.concatenate((varY, np.zeros((bs,d+1 ))), 1)
    # var_vec = np.concatenate((varY, np.zeros((bs, d + 1))), 1)
    var_vec = torch.cat([varY, torch.zeros((bs, d + 1)).to(device)], 1)
    # exp = np.einsum('ij,i->ij', y_mean_vec, meanY) + var_vec
    exp = y_mean_vec * torch.reshape(meanY,(bs,1)) + var_vec
...
def M_gradient(x, meanY, varY, y, Myy, Myx,d):
...
class regression_gradient(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_x = grad_output.clone()
        grad_x = grad_x  *(y - meanY)* M.reshape(-1, len(M))/grad_x.shape[0]
